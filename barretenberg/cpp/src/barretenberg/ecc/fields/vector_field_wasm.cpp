// VectorField operator* WASM SIMD implementation. Compiled as a separate TU
// for the WASM build only so LLVM emits each per-Params specialization as a
// standalone function (not inlined into callers), giving it the same register
// allocation scope the gist's hand-written WAT function gets. Inlining across
// ~2400 lines of carefully ordered ops lets LLVM's instruction scheduler
// re-coalesce locals and recreate the stream-reorder problem we solved with
// asm barriers.
//
// The kernel body lives in vector_field_mont_mul_body.inl.hpp and is included
// once per explicit specialization (Bn254FrParams, Bn254FqParams). The macros
// it expands (BB_VF_LOAD_LIMBS, BB_VF_KARATSUBA_STAGES_1_4,
// BB_VF_RUN_STAGES_6_THROUGH_10) reference unqualified R_INV_WASM, P_WASM,
// R_INV_MOD_2_29 — these resolve in the enclosing class scope to whichever
// Params constants the specialization was bound to.
